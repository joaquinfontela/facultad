...
static bool server_logic_init(server_t* self, int argc, char** argv) {
  ...
  encoder_t decoder;
  encoder_t_init(&decoder, (uint8_t*)(self->commandParser.method),
...
static ssize_t server_logic_recieveMessage(server_t* self, uint8_t* buf) {
...
static bool server_logic_decode(server_t* self, uint8_t* buf,
